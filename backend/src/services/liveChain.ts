...
const DERIVED_CACHE_TTL_MS = Number(process.env.CHAIN_DERIVED_CACHE_TTL_MS ?? 30_000);
  ...
  private detectParsedArbitrage(
    ...
    for (const [signer, signerSwaps] of grouped) {
      ...
      attacks.push({
        ...
        frontrun_tx: candidateWindow[0].signature,
        victim_tx: candidateWindow.length > 1 ? candidateWindow[candidateWindow.length - 1].signature : null,
        backrun_tx: null,
