...
exports.scoreWallets = scoreWallets;
exports.clusterEntities = clusterEntities;
exports.updatePoolToxicity = updatePoolToxicity;
...
async function clusterEntities() {
    ...
    const { rows: poolOverlap } = await pool_1.pool.query(`
    SELECT
  ...
  `, [mevWallets.map(w => w.wallet)]);
    ...
    const { rows: attackPairs } = await pool_1.pool.query(`
  ...
  `, [mevWallets.map(w => w.wallet)]);
    ...
    for (const [root, members] of groups) {
        const entityId = crypto_1.default
            .createHash("sha256")
            .update(root)
            .digest("hex")
            .slice(0, 16);
        ...
        // Get attack stats
        const { rows: attackStats } = await pool_1.pool.query(`
      SELECT
    ...
    `, [members]);
        ...
        // Check known labels
        const { rows: labels } = await pool_1.pool.query(`SELECT name, wallet FROM operator_labels WHERE wallet = ANY($1)`, [members]);
    ...
    `, [
      ...
      `, [entityId, wallet]);
        ...
        await pool_1.pool.query(`UPDATE mev_attacks SET entity_id = $1 WHERE attacker_wallet = ANY($2)`, [entityId, members]);
    }
...
// ============================================================
async function updatePoolToxicity() {
    await pool_1.pool.query(`
...
function computeFeeAggression(w) {
