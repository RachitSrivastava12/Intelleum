...
exports.extractTokenFlows = extractTokenFlows;
const programs_1 = require("./programs");
function keyToAddress(value) {
    if (!value)
        return null;
...
function instructionProgramId(ix, accountKeys) {
...
function preferredPoolPositions(programLabel) {
    switch (programLabel) {
        case "raydium_amm":
        case "raydium_amm_v3":
        case "raydium_clmm":
...
function isAggregatorLike(programLabel) {
    return (programLabel === "jupiter_v4" ||
        programLabel === "jupiter_v6" ||
        programLabel === "raydium_router");
}
function derivePoolAddress(instructions, accountKeys, signer, dexProgram) {
    if (!dexProgram || !instructions?.length)
        return null;
    const programLabel = (0, programs_1.getProgramLabel)(dexProgram);
    if (isAggregatorLike(programLabel))
        return null;
    ...
    for (const ix of instructions) {
        ...
        for (const [position, ref] of instructionAccounts(ix).entries()) {
            const address = typeof ref === "number"
            ...
            if (address.length < 32 || address.length > 44)
                continue;
            const writable = typeof ref === "number"
                ? Boolean(accountKeys[ref]?.writable)
                : true;
            const executable = typeof ref === "number"
...
async function extractTokenFlows(block, slot, priceMap) {
    const flows = [];
    for (const tx of block.transactions ?? []) {
        ...
        const dexPrograms = [...programIds].filter((programId) => (0, programs_1.isDex)(programId));
        const dexProgram = dexPrograms.find((programId) => !isAggregatorLike((0, programs_1.getProgramLabel)(programId))) ??
            dexPrograms[0] ??
            null;
        const lendingProgram = [...programIds].find((programId) => (0, programs_1.isLending)(programId)) ?? null;
        ...
        for (const balance of post) {
            ...
            const deltaUsd = price !== null ? (Number(delta) / Math.pow(10, decimals)) * price : null;
            flows.push({
                tx_sig: sig,
                slot,
                wallet: owner,
        ...
        const preSOL = (meta.preBalances ?? []).map((balance) => BigInt(balance));
        const postSOL = (meta.postBalances ?? []).map((balance) => BigInt(balance));
...
//# sourceMappingURL=tokenFlows.js.map