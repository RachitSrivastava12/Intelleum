...
exports.getRouteRisks = getRouteRisks;
exports.evaluateRoute = evaluateRoute;
exports.rankRoutes = rankRoutes;
exports.getRouteRecommendations = getRouteRecommendations;
exports.getLiveAlerts = getLiveAlerts;
exports.getIntegrationFeeds = getIntegrationFeeds;
exports.getWallet = getWallet;
const NOW = Date.now();
const BASE_SLOT = 329800000;
const entities = [
    {
        id: "entity_b91_alpha",
...
function sortByNewest(items) {
    return [...items].sort((a, b) => new Date(b.block_time).getTime() - new Date(a.block_time).getTime());
}
function buildStats() {
    ...
    return {
        ...
        attacks_1h: attacks.filter((attack) => Date.now() - new Date(attack.block_time).getTime() <= 60 * 60 * 1000).length,
...
function getEntities(params) {
    ...
    if (params.strategy) {
        results = results.filter((entity) => entity.strategies_used.includes(params.strategy));
    }
    if (params.min_risk) {
...
function getEntity(id) {
    ...
    if (!entity)
        return null;
    const recentAttacks = sortByNewest(attacks.filter((attack) => attack.entity_id === entity.id));
...
function getPoolDetails(address) {
    const toxicity = pools.filter((pool) => pool.pool_address === address);
    const recentAttacks = sortByNewest(attacks.filter((attack) => attack.pool_address === address));
...
function getRouteRisks(limit = 25) {
    ...
    for (const attack of attacks) {
        ...
        if (!existing) {
            grouped.set(attack.pool_address, {
                ...
                bundle_sum: attack.validator.toLowerCase().includes("jito") || (attack.tip_lamports ?? 0) >= 120000 ? 0.64 : 0.22,
        ...
        existing.bundle_sum +=
            attack.validator.toLowerCase().includes("jito") || (attack.tip_lamports ?? 0) >= 120000 ? 0.64 : 0.22;
...
function estimateBpsAtRisk(route, objective) {
...
function classifyRouteDecision(route, objective, estimatedBpsAtRisk, saferAlternativesCount) {
...
function matchRouteRisk(query, routeRisks) {
    if (query.route_key) {
...
function evaluateRoute(request) {
...
function rankRoutes(request) {
    const objective = request.objective ?? "best_execution";
    const ranked = request.candidates
        .map((candidate) => evaluateRoute({
...
function getRouteRecommendations(limit = 12) {
    const grouped = new Map();
...
function getLiveAlerts(limit = 20) {
    return sortByNewest(attacks)
        .slice(0, limit)
        .map((attack) => {
        const bundleLikelihood = attack.validator.toLowerCase().includes("jito") || (attack.tip_lamports ?? 0) >= 120000 ? 64 : 22;
...
function getIntegrationFeeds(limit = 20) {
    return {
